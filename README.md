...
- **複写ボタンによる類似ルール作成 (Rule duplication) — v1.2.0**
    - ルール一覧の各行に **[複写]** ボタンを追加。既存ルールの全項目をフォームにコピーして新規登録の準備状態にします（`[編集] [複写] [削除]` の順）。
    - 更新モード中に押下した場合は、入力中の内容を破棄するか確認した上で追加モードに切り替わります。
- **最大 50 件制限 (Rule count limit) — v1.2.0**
    - ルールの登録件数を最大 50 件に制限。50 件到達時は `[新規登録]` がエラーを表示し、複写ボタンも無効化されます。51 件以上が保存されている状態でアップデートした場合は起動時に警告を表示後、51 件目以降を自動削除します。
...
4.  登録したルールは **「ルール一覧」** に表示され、以下が可能です。
    - **[編集]** ボタンで内容を修正（入力中の変更がある場合は破棄確認ダイアログを表示）
    - **[複写]** ボタンで既存ルールをフォームにコピーして類似ルールを素早く作成（v1.2.0／更新モード中に押下した場合は追加モードへ切り替え＋破棄確認）
    - **[削除]** ボタンで削除（誤クリック防止のため常に確認ダイアログを表示。入力中の変更がある場合は破棄＋削除を一文で確認）
    - **ドラッグ操作** で優先順位（上位優先）を並び替え
    - ※ ルールは最大 50 件まで登録可能（v1.2.0）
